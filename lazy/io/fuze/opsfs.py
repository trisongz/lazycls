...
"""
Handles whether to use fuse or fuse3

Since some prereqs are OS dependent, such as compiling fuse3 for ubuntu,
we need to generate the class at runtime
"""
import os
import sys
import errno
...
def build_fuse3():
    assert _FUZE3_ALLOWED, 'Fuse3 is not supported on non-linux distros'
    
    Lib.run_bash_script('fuse3')
    Lib._ensure_lib_installed('pyfuse3')
    Lib._ensure_lib_installed('trio')
    ...
    class Operations(pyfuse3.Operations):
        ...
        def _inode_to_path(self, inode):
            try: val = self._inode_path_map[inode]
            except KeyError: raise FUSEError(errno.ENOENT)
...
def get_fuzeops(use_fuse3: bool = False):
    if not use_fuse3:
        try: return build_fuse3()
        except: pass
    _prepare_fuze()
